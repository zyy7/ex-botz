...
let handler = async (m, { conn, command }) => {
  if (/^tod$/i.test(command)) {
    conn.send3Button(m.chat, 'Truth or Dare', '© Ex-Bot', 'Truth', ',truth', 'Dare', ',dare', 'RANDOM', `${pickRandom([',dare', ',truth'])}`, m)
  }
  ...
  if (/^dare$/i.test(command)) {
    ...
    let json = await res.json()
    if (json.message == "") throw json
    conn.send2Button(m.chat, json.message, '© Ex-botz', 'Truth', ',truth', 'Dare', ',dare', m)
